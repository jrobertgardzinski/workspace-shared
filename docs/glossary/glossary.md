...
202 classes · 22 feature files tagged.
...
- **CodeChannel** — Outbound port that delivers a short one-time code to a target (an e-mail address, a phone number). `microservice-security/security-domain/src/main/java/com/jrobertgardzinski/security/domain/port/CodeChannel.java`
...
- **FactorType** — Which kind of authentication factor — a stable string id, not a domain enum, on purpose: adding a new method (TOTP, WebAuthn, a hardware token…) is a new adapter keyed by its type, and must never force an edit to a central enum. `microservice-security/security-domain/src/main/java/com/jrobertgardzinski/security/domain/vo/FactorType.java`
...
- **Meme** — A stored meme: an identifier, who uploaded it, the (browser-friendly) image format, and the image bytes. `microservice-memes/memes-domain/src/main/java/com/jrobertgardzinski/memes/domain/Meme.java` · _used in_ moderate-meme, tag-meme, upload-meme, vote-meme
...
- **ChallengeCodeConfig** — The lifecycle of an MFA challenge code (e-mail / SMS): how long a code lives, how many wrong proofs a single sign-in ticket tolerates before it is torn down, and how many digits the code has. `microservice-security/security-config/src/main/java/com/jrobertgardzinski/security/config/mfa/ChallengeCodeConfig.java`
...
- **AuthenticationUseCases** — The two halves of a sign-in: start (through link #1) and continue (through the factor chain). `microservice-security/security-system/src/main/java/com/jrobertgardzinski/security/system/authentication/AuthenticationFactory.java`
...
- **CodeHasher** — Hashes a one-time code for storage against a challenge — the raw code is never kept. `microservice-security/security-system/src/main/java/com/jrobertgardzinski/security/system/mfa/CodeHasher.java`
...
- **EmailCodeFactor** — The e-mail second factor: on #issueChallenge it mints a random numeric code, sends it through the CodeChannel (an outbox mail event in production, a capturing double in tests), and remembers only its hash and expiry. `microservice-security/security-system/src/main/java/com/jrobertgardzinski/security/system/mfa/EmailCodeFactor.java`
...
- **EnrolFactor** — Enrol a new factor for a signed-in user, in two steps that prove they control what they are adding: #start issues the factor's challenge (a code to the address/number they gave), #confirm accepts one correct proof and persists the factor. `microservice-security/security-system/src/main/java/com/jrobertgardzinski/security/system/mfa/EnrolFactor.java`
...
- `microservice-memes/memes-infrastructure/src/test/resources/features/moderate-meme.feature` — `Meme`
